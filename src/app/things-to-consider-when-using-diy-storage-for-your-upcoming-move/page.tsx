...
export async function generateMetadata() {
    const post = await getPostData('things-to-consider-when-using-diy-storage-for-your-upcoming-move');
...
export default async function DIYStoragePage() {
    const mainSlug = 'things-to-consider-when-using-diy-storage-for-your-upcoming-move';
    const relatedSlugs = [
        'cut-cost-for-home-relocation',
    ...
    // Remove the duplicate "Give us a call" section that comes from WP content
    // We want to keep only the hardcoded one at the bottom.
    // The duplicate, including its button/wrapper, appears at the end of the WP content.
    // We can safely truncate the content when we find this specific start phrase.
    const duplicatePhrase = 'Give us a call';
    const duplicateIndex2 = cleanedContent.lastIndexOf(duplicatePhrase);

    if (duplicateIndex2 !== -1) {
        // Verify it's the section we think it is (near the end)
        // and closer to the end than the beginning
        if (duplicateIndex2 > cleanedContent.length / 2) {
            // Look backwards for the preceding paragraph close to cut cleanly if possible
            const pEndBefore = cleanedContent.lastIndexOf('</p>', duplicateIndex2);
            if (pEndBefore !== -1) {
                cleanedContent = cleanedContent.substring(0, pEndBefore + 4);
            } else {
                // Fallback: just cut at the phrase
                cleanedContent = cleanedContent.substring(0, duplicateIndex2);
            }
        }
    }
                    ...
                    <div className="container mx-auto px-4 lg:px-0 max-w-6xl">
                        <div className="relative w-full bg-[#0B2341] rounded-3xl overflow-hidden flex flex-col lg:flex-row h-auto lg:h-[500px] shadow-2xl">
                            ...
                            <div className="lg:w-[60%] relative h-[300px] lg:h-full">
                                <Image
                                    src="https://wowmover.com/wp-content/uploads/2021/07/Things-To-Consider-When-Using-DIY-Storage.jpg"
                                    alt="Things To Consider When Using DIY Storage"
                                    fill
                            ...
                            <div className="lg:w-[40%] bg-[#0B2341] flex flex-col justify-center p-8 lg:pl-8 lg:pr-16 text-white relative z-30">
                                ...
                                <button className="bg-[#76D69F] hover:bg-[#65c58e] text-white font-bold py-3 px-8 rounded-full w-fit transition-colors uppercase text-sm tracking-wide">
                                    Get a Quote
                                </button>
                            </div>
                        </div>
                ...
                <div className="container mx-auto px-4 pb-12 lg:pb-16 max-w-5xl">
                    ...
                    <div className="bg-[#F0FDF4] p-4 rounded-lg mb-12 border-l-4 border-[#76D69F]">
                        <p className="font-bold text-[#0B2341] m-0 flex gap-2">
                            Also Read: <span className="text-[#76D69F] font-normal">Why DIY Moving Is a Waste of Time and a Bad Idea</span>
                        </p>
                    </div>
                    ...
                    <div className="text-center mb-20">
                        ...
                        <a href="tel:8448581971" className="inline-block bg-[#34D399] hover:bg-[#10B981] text-white font-bold py-4 px-12 rounded-md text-lg transition-colors shadow-lg shadow-green-200">
                            Call (844) 858-1971
                        </a>
                    </div>
